...
class PDF_File:
    def split_pdf_pages(
        input_pdf_path, target_dir, fname_fmt="{name}_{num_page:04d}.pdf"
    ):
        ...
        with open(input_pdf_path, "rb") as input_stream:
            ...
            for i in range(0, len(input_pdf.pages)):
                ...
                file_name = os.path.join(
                    target_dir,
                    fname_fmt.format(
                        name=Split.file_name(input_pdf_path), num_page=i + 1
                    ),
                )
    ...
    def pdf_to_image(input_pdf_path):
        ...
        images = convert_from_path(
            input_pdf_path, 300, poppler_path=settings.poppler_path
        )
        for image in images:
            imagename = f"{Split.file_name(filename)}.png"
            imagename = os.path.join(dirname, imagename)
            image.save(imagename)
            logger.info(f"Создан файл {imagename}")
        return imagename
